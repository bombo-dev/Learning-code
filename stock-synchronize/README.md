...
1. 동일한 메모리에 같은 순간에 접근하여 데이터를 조작하는 race condition 은 심심치 않게 발견 됨.
   ...
   2. **비관적 락(Exclusive Lock) 으로 동시성 문제 해결하기**
    ...
    4. **네임드 락(Named Lock) 으로 동시성 문제 해결하기** 
        - 테이블 혹은 레코드 그 자체에 락을 거는 것이 아니라 별도의 공간을 사용한다.
        - 네임드락을 사용하는 경우에는 데이터 소스를 분리해서 사용하는 것을 추천한다. 커넥션 풀이 부족해질 수 있기 때문이다.
        - 네임드락은 주로 분산락을 구현하는 경우에 사용하며, 데이터 삽입 시에 정합성을 맞춰야하는 경우에도 사용한다.
        - 단, 트랜잭션 전후로 트랜잭션 종료 시에 락 해제를 해주어야 하기 때문에 데이터베이스 세션 관리를 잘해주어야해서, 실제로는 더 복잡해질 수 있다.
        - 걸린 시간 1210ms
